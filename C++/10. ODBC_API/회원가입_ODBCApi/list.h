...
template<typename T> struct ListNode
{
	...
	ListNode(T _value, ListNode* _next, ListNode* _prev)
	{
		value = _value;
		next = _next;
		prev = _prev;
...
public:
	...
	*/
	...
	void ForEachNode(std::function<void(ListNode<T>*)> fptr) {
		ListNode<T>* pThis = first;
		while (pThis)
		{
			(fptr)(pThis); 
...
public:
	...
	int Add(T item)
	{
		if (count == 0)
		{
			ListNode<T>* newNode = new ListNode<T>(item, nullptr, nullptr);
			first = newNode;
			last = newNode;
		}
		else
		{
			ListNode<T>* newNode = new ListNode<T>(item, nullptr, last);
			last->next = newNode;
			last = newNode;
		}
		++count;
